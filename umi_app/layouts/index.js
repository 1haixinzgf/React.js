...
import { Tabs } from 'antd-mobile'; // 内置的
import * as styles from './index.less';// umi支持less语法 styles 命名空间

// 纯组件PureComponent   这里面没有外部数据，单纯的做界面输出
// 函数组件 () => (<div>j</div>)  React.createElement
// 纯组件没有太多的复杂操作
// Component 标准操作
// 性能 new
// layout? 头部和tabbar umi 约定所有页面都会默认引入layout/index.js
        ...
        { // 相当于vue中的v-if
          showTab && (
            <div className={styles.tabbar}>
              <Tabs tabs={this.tabData} onRenderContent={true} animated={false} renderTab={ tab => (
                <div className={styles.tab}>
                  <div className={styles.icon}>{ this.isActive(tab.href)?tab.icon2:tab.icon }</div>
                  <div className={styles.title} style={{color: this.isActive(tab.href)?'#0089dc':''}}>{ tab.title }</div>
                </div>
              )} tabBarUnderlineStyle={{display: 'none'}} onTabClick={tab => this.gotoTab(tab.href)}
              />
            </div>
          )
        }
